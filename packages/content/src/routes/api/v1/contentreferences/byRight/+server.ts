...
import type { ContentReferences } from '../../../../../content-references';
...
// Custom collection method: byRight
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  const typedCollection = collection as unknown as ContentReferences;
  if (!collection)
    throw error(
      500,
  ...
  const result = await typedCollection.byRight(options.rightId, options.opts);

  return json({ action: 'byRight', result: toPublicResult(result) });
};
