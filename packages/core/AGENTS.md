...
- **Transaction-bound instances**: `SmrtClass.withDatabase(db, callback)` temporarily binds an initialized instance (including its public `options.db`) to a caller-owned transaction database and restores only the database binding. Transaction owners persisting one object should use `SmrtObject.withTransaction(callback)`, which also restores identity/revision metadata after rollback so the same instance can retry. Never reach into `_db`, and do not use the same instance concurrently during either callback.
