...
import { buildContentQuerySchema } from '../content-query';
import {
...
describe('translating filters', () => {
  ...
  it('drops an operator with no sound server expression', () => {
    for (const operator of ['notContains', 'isNull', 'isNotNull'] as const) {
      const { request, dropped } = translate({
        filters: [{ columnId: 'title', operator, value: 'x' }],
      });
      expect(request.filter).toBeUndefined();
      expect(dropped).toEqual([
        {
