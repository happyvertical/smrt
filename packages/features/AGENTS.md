...
- Each row is its own `<form>`, so `formAction` gives a working screen with no client JS; `onSave` intercepts the submit instead. The global column is opt-in (`showGlobal` / `globalEditable`) because most apps have no platform-admin tier.
