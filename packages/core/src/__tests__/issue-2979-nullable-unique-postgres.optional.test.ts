 ...
 *
 * SQLite runs in the default lane (the default include also matches
 * `*.optional.test.ts`); PostgreSQL only when `SMRT_TEST_POSTGRES_URL` is set
 * (`pnpm test:postgres`).
 */
  ...
  describe.skipIf(!enabled)(`nullable uniqueness on ${engine} (#2979)`, () => {
    ...
    it.skipIf(engine !== 'postgres')(
      ...
      async () => {
        ...
        // The NULL-equal renderer falls back to a plain unique index below
        // PostgreSQL 15, where the clause does not exist.
        const versionResult = (await db.query(
          `SELECT current_setting('server_version_num')::integer AS v`,
        )) as unknown as { rows?: Array<{ v: number }> } | Array<{ v: number }>;
        const versionRows = Array.isArray(versionResult)
          ? versionResult
          : (versionResult.rows ?? []);
        if (Number(versionRows[0]?.v) >= 150000) {
          expect(conflict?.indexdef).toMatch(/NULLS NOT DISTINCT/);
        } else {
          expect(conflict?.indexdef).not.toMatch(/NULLS NOT DISTINCT/);
        }
