...
async function snapshot(
  ...
  const queries: Record<string, string> = {
    ...
    rewriteRules: `SELECT n.nspname AS schema, c.relname AS table_name, r.rulename AS rule_name FROM pg_rewrite r JOIN pg_class c ON c.oid=r.ev_class JOIN pg_namespace n ON n.oid=c.relnamespace WHERE r.rulename <> '_RETURN' ORDER BY n.nspname,c.relname,r.rulename`,
...
async function plan(
  ...
  for (const rule of state.rewriteRules)
    if (rule.schema === contract.schema)
      unsupported(
        'user-rewrite-rule',
