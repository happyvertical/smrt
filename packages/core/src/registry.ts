...
// Import test manifest (generated by pretest script)
// Falls back to stub if not generated
import { testManifest } from './manifest/test-manifest-stub';
...
export class ObjectRegistry {
  ...
  static register(
    ...
    // Get field definitions from AST manifest
    // Check test manifest first (for test classes), then static manifest (for production classes)
    const manifestEntry =
      testManifest?.objects[name.toLowerCase()] ||
      staticManifest.objects[name.toLowerCase()] ||
      testManifest?.objects[name] ||
      staticManifest.objects[name];
