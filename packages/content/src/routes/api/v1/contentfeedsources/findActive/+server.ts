...
import type { ContentFeedSourceCollection } from '../../../../../content-feed-sources';
...
// Custom collection method: findActive
export const POST: RequestHandler = async ({ locals, request }) => {
  requireRouteAuth(locals, true);
  establishTenantContext(locals);
  const collection = await getCollection<ContentFeedSource>(
    '@happyvertical/smrt-content:ContentFeedSource',
  );
  const typedCollection = collection as unknown as ContentFeedSourceCollection;
  if (!collection)
    throw error(
      500,
  ...
  const result = await typedCollection.findActive(options.tenantId);

  return json({ action: 'findActive', result: toPublicResult(result) });
};
