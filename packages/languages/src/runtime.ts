 ...
 *
 * Browser-safe, dependency-free language helpers. This module imports no
 * `node:*` builtins and none of the package's server/worker dependencies
 * (smrt-core, sql, ai, smrt-jobs, …), so it is safe to bundle into a client
 * (e.g. the smrt-svelte i18n layer). It is the single source of truth for the
 * interpolation and locale-matching contract that both the server resolver and
 * the client renderer share.
 *
 * The full read path (`resolveLanguageString`, the registry, overrides) lives
 * on the package root; the worker stack on `/jobs`; admin CLI on `/cli`.
 *
