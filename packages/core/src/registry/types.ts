...
export interface ApiConfig {
  /**
   * Optional collection path override for the **agent-facing route map** built
   * by `@happyvertical/smrt-agents` (`buildRouteMap`), which derives a
   * resource path as `api.path ?? tableName.replace(/_/g, '-')`. It is the
   * supported way to keep legacy/public route shapes stable there when the
   * storage table name differs from the desired URL segment.
   *
   * It does NOT affect SvelteKit routes generated by the Vite plugin: those
   * take their collection segment from the manifest's `collection` verbatim,
   * as does the runtime dispatcher in `generators/rest.ts`. Use
   * `routes[action].path` to shape an individual generated route (#2630).
   */
  path?: string;
...
export interface SmartObjectConfig {
  ...
  mcp?:
    ...
    | {
         ...
         *
         * Entries are matched EXACTLY and are case-sensitive, against the name
         * as declared — the lowercase verb for a CRUD action (`list`), the
         * method's own casing for a custom action (`syncNow`). An entry that
         * differs only in case from a CRUD verb cannot be honored: it names no
         * CRUD action, and exposing it as a custom action would build the
         * reserved `${object}_${verb}` tool, which dispatches to the built-in
         * operation rather than the class's method. Such an entry is discarded
         * with a warning (#2646).
         */
        include?: string[];
