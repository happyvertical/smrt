...
| Model | Purpose |
|---|---|
| `Sku` | Smallest sellable / countable unit. Plain-string `productId` links to upstream product catalog. JSON `attributes` carries axis values. `conflictColumns: ['code', 'tenant_id']`. |
| `Variant` | Declarative axis definition (`axisName: 'size'`, `values: [...]`). Drives UI / forms; per-SKU values live in `Sku.attributes`. `conflictColumns: ['product_id', 'axis_name', 'tenant_id']`. |
...
- **Cross-industry constraint.** This package's vocabulary stays generic: `Sku`, `Variant`, `InventoryLocation`. Apparel-specific concepts (`Style`, `Makeup`, `Colorway`, fashion `Season`, tech-pack metadata) and their analogues in furniture / automotive / CPG live in the relevant template package, never here. PRs that introduce industry vocabulary should be rejected.
- **Cross-package references are plain strings.** `productId`, `placeId`, `parentSkuId`, `sourceId` — all plain string ids, never `@foreignKey()`. Keeps the dependency graph DAG-shaped and lets each upstream package evolve independently.
- **`conflictColumns` include `tenant_id`** on `Sku`, `Variant`, `InventoryLocation`, and `StockLevel`. This makes the natural keys per-tenant unique while letting global rows (`tenant_id = null`) coexist with tenant-scoped rows of the same code. NULL-matching semantics (`(code, NULL) IS NOT DISTINCT FROM (code, NULL)`) are handled by `@happyvertical/sql >= 0.74.0` natively, so two saves with the same `(code, NULL)` tuple correctly merge in place. Pass `nullsDistinct: true` at the sql layer to opt back into NULL-distinct semantics.
- **JSON columns (`Sku.attributes`, `Variant.allowedValues`)** are stored as text. Use the `getX()` / `setX()` helpers so callers don't have to wrap reads in try/catch. `allowedValues` is named with the prefix to dodge the SQL `VALUES` reserved word.
