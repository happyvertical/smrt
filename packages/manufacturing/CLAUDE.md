...
- **`consumeMaterials` is not atomic across BOM lines.** Each line is its own `StockService.adjust` call — if line N+1 throws after line N succeeded, line N's materials are gone but the production run hasn't fully consumed. The same "split-write atomicity" gap that affects `StockService` itself (see `@happyvertical/smrt-inventory` CLAUDE.md) compounds here across lines. The current mitigation is the recommended pre-flight: `BomService.canProduce(orderId, qty)` checks every line's available stock before the run starts. Drive runs through `@happyvertical/smrt-jobs` for retry on transient failures. A future release will ship `ProductionService.withTransaction(fn)` once the underlying transaction API stabilizes.
