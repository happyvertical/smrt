...
| --- | --- | --- | --- |
| Test-manifest generation does not affect the production build hash | Generate a test manifest in a copied core package under a temporary workspace, then ask Turbo for the copied package's normal dry build graph | The cold and generated hashes match; the graph includes core's `generate` prerequisite, and changing a fixture-only `generate` input changes the core build hash; the base test invoked `pnpm --dir <live core> run build`, so safely reproducing its mutation in a concurrent run is infeasible | Integration: `pnpm --filter @happyvertical/smrt-core exec vitest run src/manifest/__tests__/issue-2223-task-ownership.test.ts` |
