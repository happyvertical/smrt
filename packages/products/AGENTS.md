...
- **`price` and every money/cost field must initialize `0.0`, never `0`** — a bare integer literal compiles the shared `products.price` column to INTEGER, and PostgreSQL then rejects `19.99` with `22P02` while SQLite's affinity stores it and hides the bug from this package's SQLite suites. `Product.price` carried that until #2361; `pnpm --filter @happyvertical/smrt-products test:postgres` is the lane that holds the line. Because `Product` is the STI base, the column is shared by every consumer subtype.
