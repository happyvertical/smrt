...
- `FeatureSettingsService` — read/write helper for a management screen: `listFeatureSettings()` returns one row per definition (definition fields, `effectiveEnabled`, `globalEffect`, `tenantEffect`), `setFeatureOverride()` refuses unknown keys then delegates to `FeatureOverrideService` (#3052)
...
- Two invariants the selects have to keep, both regression-tested: the tenant column's *Default* option names the **inherited** state (`inheritedEnabled()`, i.e. the global override when there is one), never the bare code default, or it tells an operator the opposite of what choosing it does; and the rows remount on `contextKey ?? features` so an unsaved choice cannot survive a reload or a tenant switch and be submitted against the new rows.
