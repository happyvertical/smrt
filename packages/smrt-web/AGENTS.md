...
| `offline/` | durable offline writes — config, sync-apply or consumer-declared transport replay (#3021), idempotency, the shared namespace-keyed engine, and Web Locks leader election | [agents/offline-outbox.md](agents/offline-outbox.md) |
...
| `persistence/` + `update-state.ts` | the read-cache rehydrate capability, `persistDataSurface` (#3021), and the framework-free `updateAvailable` primitive (bundle + contract signals) | [agents/version-persistence.md](agents/version-persistence.md) |
...
| `ai.ts` | the browser inference path — ONE call-site object over interchangeable inference backends, with resolution **per call** so a preference change applies to the next `chat`/`stream` and never to one already in flight. `InferenceBackend`/`InferenceMessage`/`InferenceResponse`/`InferenceChatOptions` are textual structural mirrors of `@happyvertical/ai`'s `AIMessage`/`AIResponse`/`ChatOptions` (text generation only), and `InferenceProgress` mirrors the browser adapters' `DownloadProgressInfo` so a `DownloadProgress` control renders it directly. `createRouteInferenceBackend` consumes `@happyvertical/smrt-chat`'s `ChatStreamEvent` frame vocabulary without depending on that package. Backends arrive as arguments; a WebLLM backend lives beside the adapter it wraps. Ships as the dependency-free `@happyvertical/smrt-web/ai` entry — importing it pulls no client-data engine | — |
