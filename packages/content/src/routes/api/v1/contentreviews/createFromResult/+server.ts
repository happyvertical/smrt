...
import type { ContentReviewCollection } from '../../../../../content-reviews';
...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  const collection = await getCollection<ContentReview>(
    '@happyvertical/smrt-content:ContentReview',
  );
  const typedCollection = collection as unknown as ContentReviewCollection;
  if (!collection)
    throw error(
      500,
  ...
  return json({ action: 'createFromResult', result: toPublicResult(result) });
};
