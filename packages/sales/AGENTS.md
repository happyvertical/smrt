...
- **CommissionPlan**: versioned calculation terms. Natural key `(plan_key, version)`; `status` (`draft|active|superseded|retired`); `effectiveFrom`. Components are a JSON-string array (`getComponents()`): each component has `key`, `trigger` (earning-event kind or `*`), `basis` (`fixed|gross|net|margin|custom`), `rate` or `fixedAmountCents`, `recurrence` (`one_time` or `recurring` with optional `maxOccurrences`/`windowMonths`). **Immutable once active** — amendments create a new row with `version + 1`; prior versions are never rewritten.
...
- **CommissionPayout**: settlement batch per Earner. `pending → approved → processing → completed | failed`; settles exact Commission/Adjustment rows by stamping `payoutId`; idempotent via natural key; retains `paymentReference`/provider references; optional `invoiceId` cross-package string ref to a commerce Invoice.
...
- **AttributionPolicy**: versioned `(policy_key, version)` policy — attribution `windowDays`, credit mode (`first_touch|last_touch|assigned|split`), split shares, self-referral/existing-client eligibility, eligible services/campaigns/regions, conflict behavior. Immutable once active; amendments bump `version`.
...
- **Services**: `AttributionService` (resolve touches → Referral(s) or exception, overrides), `ReferralQualificationService` (eligibility + snapshot + transition), `ReferralCommissionService` (bridge: qualified Referral + EarningEvent → Commissions through the snapshot, idempotently).
