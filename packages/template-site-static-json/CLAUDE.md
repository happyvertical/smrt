...
- **Workflow scripts**: `workflow:caelus` and `workflow:praeco` both delegate to upstream CLIs via `npx <pkg> --config smrt.config.js`. If the upstream microservice has not yet shipped a `bin` entry, scaffolded projects will need to fall back to direct `tsx` invocation pointed at the upstream `src/workflows/*.ts` until the CLI is exposed.
