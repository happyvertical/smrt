import { O as ObjectRegistry } from "../chunks/registry-Cr7Ueogq.js";
...
function generateObjectSchema(objectName) {
...
function fieldToOpenAPISchema(field) {
  ...
  switch (field.type) {
    ...
    case "integer":
      schema.type = "integer";
      if (field.options?.min !== void 0) schema.minimum = field.options.min;
      if (field.options?.max !== void 0) schema.maximum = field.options.max;
      break;
    case "decimal":
      schema.type = "number";
      schema.format = "float";
...
function setupSwaggerUI(app, spec, path = "/docs") {
  try {
    ...
    app.get(`${path}/openapi.json`, (_req, res) => {
      res.json(spec);
    });
