...
export interface ApplyMigrationsOptions {
  ...
  reconcile?: boolean;
  /**
   * Enable PostgreSQL concurrent-index mode.
   *
   * On the non-atomic path this routes index DDL outside the transaction and
   * bounds it with session `lock_timeout`/`statement_timeout`.
   *
   * On the atomic path (`atomic: true`) it splits the batch: non-index DDL
   * still commits in one transaction, then each index statement runs
   * `CONCURRENTLY` on a pinned session afterwards. **Concurrent mode is not
   * atomic** — the committed non-index DDL survives a later index failure, and
   * unfinished index migrations are recorded `failed` so a reconciling re-run
   * retries them.
   *
   * Ignored on non-PostgreSQL engines. Timeouts are applied to the atomic
   * PostgreSQL path regardless of this flag.
   */
...
export interface MigrationsConfig {
  ...
  postgres?: {
    /** Allow CONCURRENTLY index builds under `db:migrate --postgres-safe` (default: true) */
    useConcurrently?: boolean;
    /** `lock_timeout` for every PostgreSQL migration statement (default: '30s') */
    lockTimeout?: string;
