...
- **Attribution resolution is single-writer per (target, program)**: `resolve()` is idempotent against PERSISTED state, but two workers resolving the same target concurrently can both pass the existence check and create duplicate credit — a Referral has no natural key to dedup an in-flight insert on (unlike a payout claim, which is an atomic guarded UPDATE on an existing row). Run intake resolution serially per target (it naturally is, in a request handler); duplicates are visible in the portal and correctable via `override()`.
