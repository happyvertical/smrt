...
function getCommitsSinceBase() {
  const commits = execCommand(
    `git log ${baseBranch}..HEAD --pretty=format:"%H|||%s|||%b"`,
  );
...
function generateChangesetContent(_commits, parsedCommits) {
  ...
  for (const parsed of parsedCommits) {
    for (const pkg of parsed.packages) {
      ...
      if (
        newBump === 'minor' ||
        (newBump === 'patch' && currentBump === 'patch')
      ) {
...
function main() {
  ...
  for (const parsed of parsedCommits) {
    console.log(
      `  ${parsed.type}(${parsed.scope || 'all'}): ${parsed.description}`,
    );
    console.log(
      `    → ${parsed.packages.length} package(s), ${parsed.bumpType} bump`,
    );
  }
