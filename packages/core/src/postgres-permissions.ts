...
async function snapshot(
  ...
  const queries: Record<string, string> = {
    ...
    rewriteRules: `SELECT n.nspname AS schema, c.relname AS table_name, r.rulename AS rule_name, referenced_schema.nspname AS referenced_schema, referenced.relname AS referenced_table FROM pg_rewrite r JOIN pg_class c ON c.oid=r.ev_class JOIN pg_namespace n ON n.oid=c.relnamespace JOIN pg_depend d ON d.classid='pg_rewrite'::regclass AND d.objid=r.oid AND d.refclassid='pg_class'::regclass JOIN pg_class referenced ON referenced.oid=d.refobjid JOIN pg_namespace referenced_schema ON referenced_schema.oid=referenced.relnamespace WHERE r.rulename <> '_RETURN' ORDER BY n.nspname,c.relname,r.rulename,referenced_schema.nspname,referenced.relname`,
